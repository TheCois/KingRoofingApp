(function ($) {
    $.fn.abcGrid = function (options) {
        ...
        $("<div id='tbody' class='tbody scrollbox'></div>").appendTo(this);
        $('.tbody').css("height", settings.gridHeight);
        //this loop is for heading only	
        for (i = 0; i < numberOfKeys; i++) {
            $("<div id='headCol" + i + "' class='tcol' style='width:calc(" + settings.columnWidth[i] + " - 8px)'><span class='dataCol_DataHead'>" + settings.columnName[i] + "</span><a id='"+settings.columnName[i].replace(/\s/g, '')+"' class='short_arrow' href='#'></a></div>").appendTo("#abcGrid_Row");
        ...
        for (var i = 0; i < totalRecord; i++) {
            ...
            myD.push([settings.dataSource[i].TasksName, settings.dataSource[i].DueDate, settings.dataSource[i].Time, settings.dataSource[i].Movement_Type, settings.dataSource[i].vdate, settings.dataSource[i].Time]);
        }

        var theTable = "";
        for (var j = 0; j < totalRecord; j++) {
            theTable += "<div id='dataRow_" + j + "' class='dataRow'>";
